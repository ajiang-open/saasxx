...
public class QueryAppenderImpl implements QueryAppender {
	...
	static final Pattern QUERY_PATTERN = Pattern.compile("(')"
			+ "|((^\\s*and\\s+)" + "|(\\s+and\\s*$))" + "|((^\\s*or\\s+)"
			+ "|(\\s+or\\s*$))" + "|((^\\s*,)|(,\\s*$))" + "|(^\\s*where\\s+)"
			+ "|(^\\s*select\\s+)" + "|(^\\s*having\\s+)"
			+ "|(^\\s*order\\s+by\\s+)" + "|(^\\s*group\\s+by\\s+)"
			+ "|(\\?(\\d)*)", Pattern.CASE_INSENSITIVE);
	static final Pattern QUERY_ARG_PATTERN = Pattern.compile("(:\\s*\\w+)"
			+ "|(\\{\\s*\\w+\\s*\\})" + "|(\\?\\d*)", Pattern.CASE_INSENSITIVE);
	...
	public QueryAppenderImpl(FinderImpl finderImpl, String queryString,
			Object[] args) {
		args = args == null ? Lang.EMPTY_ARRAY : args;
	...
	private void checkAndFix() {
		if (queryString == null) {
			throw new IllegalArgumentException(
					String.format("The append query string should not be null"));
		}
		...
		while (matcher.find()) {
			...
			if (group.contains("'")) {
				throw new IllegalArgumentException(
						String.format("The append query string should not contains \"'\""));
	...
	public QueryContent toQueryContent() {
		...
		while (matcher.find()) {
			String group = matcher.group();
			if (group.startsWith("{")) {
				String alias = aliasNameMap.get(group
						.subSequence(1, group.length() - 1).toString().trim());
				if (alias == null) {
					throw new IllegalStateException(String.format(
							"The alias which mapped %s is not exists", group));
				}
				...
				if (group.startsWith(":")) {
					...
					String name = finderImpl.finderHandler.generateParamName();
					arg = argIndexMap.get(Integer.valueOf(group.substring(1)
							.trim()));
					argMap.put(name, arg);
					matcher.appendReplacement(sb, ":".concat(name));
				}
				if (arg == null) {
					throw new IllegalStateException(String.format(
							"The arg which mapped %s is not exists", group));
				}
			}
		}
		matcher.appendTail(sb);
