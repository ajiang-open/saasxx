...
public class FinderHandler {
	...
	<T> T proxy(final Object rootProxy, final Class<T> type) {
		...
		return Proxys.newProxyInstance(new InvocationHandler() {
			...
			public Object invoke(Object proxy, Method method, Object[] args)
				...
				if (propertyName == null) {
					return new UnsupportedOperationException(String.format(
							"The method %s of object %s@%d is unsupported",
							method.getName(), proxy.getClass()
				...
				if (returnType.isArray()) {
					...
					Object subProxy = subProxy(rootProxy, proxy, propertyName,
							componentType);
				...
				if (List.class.isAssignableFrom(returnType)) {
					Class<?> componentType = Mirrors.getGenricReturnType(
							method, 0);
					final Object subProxy = subProxy(rootProxy, proxy,
			...
			private Object subProxy(final Object rootProxy, Object proxy,
					String propertyName, Class<?> componentType) {
				if(Lang.isBaseType(componentType)){
					return null;
				}
				Object subProxy = cacheProxyMap.get(propertyName);
				if (subProxy == null) {
					subProxy = proxy(rootProxy == null ? proxy : rootProxy,
							componentType);
	 ...
	 * @param propertyName
	 */
	private void setPathInfo(Object rootProxy, Object proxy,
		...
		if (pathInfo != null) {
			if (rootProxy == null) {
				...
				pathInfo = null;
			} else if (System.identityHashCode(rootProxy) != System
					.identityHashCode(pathInfo.getRootProxy())) {
		...
		if (pathInfo == null) {
			pathInfo = new PathInfo(proxy, proxy,
					new StringBuilder().append(propertyName), getter);
		} else {
